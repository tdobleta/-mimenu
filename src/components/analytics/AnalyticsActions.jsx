...
import { resetAnalytics } from '@/lib/salesOperations';

export default function AnalyticsActions({ resetLabel = 'Reiniciar analiticas', editLabel = 'Editar analitica' }) {
  ...
  const branchName = isTodas
    ? 'Todas las sucursales'
    : (store.sucursales?.find(s => s.id === store.branchId)?.nombre || '');
  ...
  async function handleReset() {
    ...
    try {
      const result = await resetAnalytics({
        ...
        reason: 'Reinicio manual de analiticas',
        user,
        role,
      });
      if (store.refreshCharts) await store.refreshCharts(store.branchId);
      addToast(`Analiticas reiniciadas (${result?.affected_count || 0} ventas archivadas)`, 'success');
      setShowReset(false);
    } catch (err) {
      console.error(err);
      addToast(err?.message || 'Error al reiniciar analiticas', 'error');
    }
    setLoading(false);
  }
    ...
    <>
      <div style={{ display: 'flex', gap: 6 }}>
        ...
        <button
          ...
          disabled={isTodas}
          title={isTodas ? 'Selecciona una sucursal especifica para reiniciar sus analiticas' : ''}
          style={{
            ...btnBase,
            color: isTodas ? '#9CA3AF' : '#EF4444',
            borderColor: isTodas ? 'rgba(0,0,0,0.08)' : 'rgba(239,68,68,0.25)',
            cursor: isTodas ? 'not-allowed' : 'pointer',
            opacity: isTodas ? 0.6 : 1,
          }}
        >
