...
export default function RestauranteTab() {
  ...
  const F = ({ k, label }) => (
    <div>
      ...
      <input value={form[k]} onChange={e=>setForm(f=>({...f,[k]:e.target.value}))} style={{ width:'100%', padding:'8px 10px', border:'1px solid #E2E8F0', borderRadius:8, fontSize:13 }} />
    </div>
  );
  ...
  return (
    <div style={{ maxWidth:460, display:'flex', flexDirection:'column', gap:14 }}>
      ...
      <div>
        ...
        <div style={{ border:'1.5px dashed #D1D5DB', borderRadius:10, padding:'24px 16px', textAlign:'center', display:'flex', flexDirection:'column', alignItems:'center', gap:6, cursor:'pointer' }}
      ...
      <div style={{ display:'flex', justifyContent:'flex-end', alignItems:'center', gap:12 }}>
        ...
        <button onClick={save} style={{ padding:'8px 20px', border:'none', borderRadius:8, fontSize:13, color:'white', backgroundColor:'#1D9E75', cursor:'pointer' }}>Guardar cambios</button>
      </div>
    </div>
  );
}
