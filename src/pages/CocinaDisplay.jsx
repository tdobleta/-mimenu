...
export default function CocinaDisplay() {
  ...
  const [estadosLocales, setEstadosLocales] = useState(() => {
    try { return JSON.parse(localStorage.getItem(`cocina_estados_auth`) || '{}'); } catch { return {}; }
  });
  ...
  useEffect(() => {
    try { localStorage.setItem(`cocina_estados_auth`, JSON.stringify(estadosLocales)); } catch {}
  }, [estadosLocales]);

  async function loadCocina() {
    if (!activeBranchId) return;
    try {
      const turns = await base44.entities.Turn.filter({ status: 'abierta', enviado_cocina: true, branch_id: activeBranchId }, '-opened_at', 50);
  ...
  function cambiarEstado(turnId, nuevoEstado) {
    ...
    // Sincronizar con DB para que el Salón pueda mostrar el punto amarillo
    if (nuevoEstado === 'lista') {
      base44.entities.Turn.update(turnId, { comanda_lista: true }).catch(() => {});
    } else if (nuevoEstado === 'preparando' || nuevoEstado === 'nueva') {
      base44.entities.Turn.update(turnId, { comanda_lista: false }).catch(() => {});
    }
    if (nuevoEstado === 'lista') {
    ...
    <div style={{ minHeight:'100vh', backgroundColor:'#0D1117', color:'white' }}>
      ...
      <div style={{ position:'sticky', top:0, zIndex:10, backgroundColor:'#0D1117', borderBottom:'1px solid rgba(255,255,255,0.08)', padding:'14px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, flexWrap:'wrap' }}>
        <div style={{ display:'flex', alignItems:'center', gap:14, minWidth:0 }}>
          ...
          <span style={{ fontSize:14, color:'rgba(255,255,255,0.5)', fontWeight:500, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{restaurante?.nombre || ''}</span>
        </div>
